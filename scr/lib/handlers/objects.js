...
export async function handleUpdateObject(request, env, headers, params) {
  ...
  // Whitelist захист
  const invalidField = changed.find((f) => !ALLOWED_PATCH_FIELDS.has(f));
  if (invalidField) return error("Заборонене поле: " + invalidField, headers, 400);
