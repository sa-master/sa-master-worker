...
import { statusLabel, isValidStatus, withStatusLabel } from "../lib/statuses.js";
...
/* =========================================================
 * POST /
 * Створення нової заявки (публічний)
 * ========================================================= */
export async function handleCreateRequest(request, env, headers) {
  ...
  // Транзакційно: request + client + events
  let requestId;
  let requestCode;
  let clientId = null;

  try {
    // 1) Генеруємо request_code через sequences (без race condition)
    ...
    if (!seqResult || typeof seqResult.value !== "number") {
      return error("Не вдалося згенерувати код заявки", headers, 500);
    }
    requestCode = `SM-R-${CURRENT_YEAR}-${String(seqResult.value).padStart(3, "0")}`;

    // 2) Клієнт: шукаємо або створюємо
    ...
    if (existingClient) {
      ...
      clientId = clientInsert.meta?.last_row_id || null;
    }
    ...
    // UNIQUE constraint на клієнта → повторити клієнт
  ...
  // Telegram — поза транзакцією, не блокує відповідь
  const telegramText = [
    "🏠 НОВА ЗАЯВКА",
    `🆔 ID: ${requestCode}`,
    `👤 Ім'я: ${name}`,
  ...
  // Не чекаємо — Cloudflare Workers підтримує waitUntil
  const tgPromise = sendTelegram(env, telegramText);
  if (typeof globalThis.waitUntil === "function") globalThis.waitUntil(tgPromise);
 ...
 * Зміна статусу заявки + синхронізація об'єкта
 * ========================================================= */
export async function handleUpdateStatus(request, env, headers, params) {
  ...
  if (!isValidStatus(newStatus)) {
    return error("Невідомий статус", headers, 400, {
      allowed_statuses: Object.keys(statusLabel ? (await import("../lib/statuses.js")).STATUS_LABELS : {}),
    });
  }
 ...
 * Прив'язка клієнта до заявки (ідемпотентно)
 * ========================================================= */
export async function handleAttachClient(request, env, headers, params) {
  ...
  // Якщо вже прив'язаний — повертаємо існуючого
  if (req.client_id) {
    ...
    return json(
      { ok: true, created: false, existing: true, request: req, client },
      headers
    );
  }
